...
- Tests: `make test-ai` (C++ AI logic and config-file parser), `python -m pytest tests/race_pipeline tests/mapgen tests/lobby` (lobby tests need Flask).
- Performance: `tests/cpp/perf_smoke.sh [units] [seconds] [warmup] [map]` (headless server, `spawn` + `perf`).
