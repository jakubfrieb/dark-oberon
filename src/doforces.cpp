...
bool TFORCE_UNIT::CanHide(TMAP_UNIT *unit, bool write_msg, bool auto_call)
{
  if (unit->GetPlayer() != myself) {
    if (write_msg) MessageText(false, "Can not hide into enemy unit.");
    return false;
  }

  if (unit->TestState(US_DYING) || unit->TestState(US_ZOMBIE) || unit->TestState(US_DELETE)) {
    if (write_msg) MessageText(false, "Can not hide into destroyed %s.", unit->GetPointerToItem()->name);
    return false;
  }

  if (unit->TestState(US_IS_BEING_BUILT)) {
    if (write_msg) MessageText(false, "Can not hide into the %s.", unit->GetPointerToItem()->name);
    return false;
  }

  if (!unit->AcceptsHidedUnits()) {
    if (write_msg) MessageText(false, "%s does not accept units.", unit->GetPointerToItem()->name);
    return false;
  }
