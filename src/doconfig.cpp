...
void LoadCfgResolution(void)
{
  ...
  config.file->ReadStr(value, "resolution", CFG_DEF_RESOLUTION, true);
  ...
  if (strcmp(value, "640x480")
      ...
      && strcmp(value, "1920x1080"))
  {
    Warning(LogMsg("Invalid resolution value '%s'. Possible values are: 640x480 | 800x600 | 1024x768 | 1152x864 | 1280x1024 | 1600x1200 | 1920x1080", value));
...
void LoadCfgTextureFilter(void)
{
  ...
  config.file->ReadStr(value, "texture_filter", CFG_DEF_TEXTURE_FILTER, true);
...
void LoadCfgMipmapFilter(void)
{
  ...
  config.file->ReadStr(value, "mipmap_filter", CFG_DEF_MIPMAP_FILTER, true);
...
void WriteDefConfig(void)
{
  ...
  config.file->WriteLine("#  Dark Oberon - configuration file");
  config.file->WriteLine("#");
  config.file->WriteLine("");

  // display
  config.file->WriteLine("# *** Display ***");
  config.file->WriteBool("fullscreen", CFG_DEF_FULLSCREEN);
  config.file->WriteStr("resolution", CFG_DEF_RESOLUTION);
  config.file->WriteBool("vert_sync", CFG_DEF_VERT_SYNC);
  config.file->WriteLine("");

  // graphics
  config.file->WriteLine("# *** Graphics ***");
  config.file->WriteStr("texture_filter", CFG_DEF_TEXTURE_FILTER);
  config.file->WriteStr("mipmap_filter", CFG_DEF_MIPMAP_FILTER);
  config.file->WriteInt("warfog_color", CFG_DEF_WARFOG_COLOR_R);
  config.file->WriteInt("warfog_color", CFG_DEF_WARFOG_COLOR_G);
  config.file->WriteInt("warfog_color", CFG_DEF_WARFOG_COLOR_B);
  config.file->WriteInt("warfog_intensity", CFG_DEF_WARFOG_ALPHA);
  config.file->WriteBool("show_fps", CFG_DEF_SHOW_FPS);
  config.file->WriteBool("show_process_fps", CFG_DEF_SHOW_PROCESS_FPS);
  config.file->WriteBool("show_disconnect_warning", CFG_DEF_SHOW_DISCONNECT_WARNING);
  config.file->WriteInt("max_frame_rate", CFG_DEF_MAX_FRAME_RATE);
  config.file->WriteInt("map_move_speed", CFG_DEF_MAP_MOVE_SPEED);
  config.file->WriteInt("map_zoom_speed", CFG_DEF_MAP_ZOOM_SPEED);
  config.file->WriteLine("");
  
  //sounds
  config.file->WriteLine("# *** Audio ***");
  config.file->WriteInt("snd_master_volume", CFG_DEF_SND_MASTER_VOLUME);
  config.file->WriteInt("snd_menu_music_volume", CFG_DEF_SND_MENU_MUSIC_VOLUME);
  config.file->WriteInt("snd_menu_sound_volume", CFG_DEF_SND_MENU_SOUND_VOLUME);
  config.file->WriteInt("snd_game_music_volume", CFG_DEF_SND_GAME_MUSIC_VOLUME);
  config.file->WriteInt("snd_game_sound_volume", CFG_DEF_SND_GAME_SOUND_VOLUME);
  config.file->WriteBool("snd_menu_music", CFG_DEF_SND_GAME_MUSIC);
  config.file->WriteBool("snd_game_music", CFG_DEF_SND_UNIT_SPEECH);
  config.file->WriteBool("snd_unit_speech", CFG_DEF_SND_MENU_MUSIC);
  config.file->WriteLine("");

  // customize
  config.file->WriteLine("# *** Customize ***");
  config.file->WriteStr("player_name", CFG_DEF_PLAYER_NAME);
  config.file->WriteStr("address", CFG_DEF_ADDRESS);
  config.file->WriteInt("sensitivity", CFG_DEF_SENSITIVITY);
  config.file->WriteLine("");
...
bool LoadConfig(void)
{
  ...
  // display
  config.file->ReadBool(&config.fullscreen, "fullscreen", CFG_DEF_FULLSCREEN);
  config.file->ReadBool(&config.vert_sync, "vert_sync", CFG_DEF_VERT_SYNC);
  LoadCfgResolution();
  ...
  config.file->ReadByteRange(config.warfog_color, "warfog_color", 0, 255, CFG_DEF_WARFOG_COLOR_R);
  config.file->ReadByteRange(config.warfog_color+1, "warfog_color", 0, 255, CFG_DEF_WARFOG_COLOR_G);
  config.file->ReadByteRange(config.warfog_color+2, "warfog_color", 0, 255, CFG_DEF_WARFOG_COLOR_B);
  config.file->ReadByteRange(&config.warfog_intensity, "warfog_intensity", 0, 100, CFG_DEF_WARFOG_ALPHA);

  config.file->ReadBool(&config.show_fps, "show_fps", CFG_DEF_SHOW_FPS);
  config.file->ReadBool(&config.show_process_fps, "show_process_fps", CFG_DEF_SHOW_PROCESS_FPS);
  config.file->ReadBool(&config.show_disconnect_warning, "show_disconnect_warning", CFG_DEF_SHOW_DISCONNECT_WARNING);
  config.file->ReadIntRange(&config.max_frame_rate, "max_frame_rate", 1, 1000, CFG_DEF_MAX_FRAME_RATE);
  config.file->ReadByteRange(&config.map_move_speed, "map_move_speed", 1, 100, CFG_DEF_MAP_MOVE_SPEED);
  config.file->ReadByteRange(&config.map_zoom_speed, "map_zoom_speed", 1, 100, CFG_DEF_MAP_ZOOM_SPEED);

  // audio
  config.file->ReadByteRange(&config.snd_master_volume, "snd_master_volume", 0, 100, CFG_DEF_SND_MASTER_VOLUME);
  config.file->ReadByteRange(&config.snd_game_music_volume, "snd_game_music_volume", 0, 100, CFG_DEF_SND_GAME_MUSIC_VOLUME);
  config.file->ReadByteRange(&config.snd_game_sound_volume, "snd_game_sound_volume", 0, 100, CFG_DEF_SND_GAME_SOUND_VOLUME);
  config.file->ReadByteRange(&config.snd_menu_music_volume, "snd_menu_music_volume", 0, 100, CFG_DEF_SND_MENU_MUSIC_VOLUME);
  config.file->ReadByteRange(&config.snd_menu_sound_volume, "snd_menu_sound_volume", 0, 100, CFG_DEF_SND_MENU_SOUND_VOLUME);
  
  config.file->ReadBool(&config.snd_unit_speech, "snd_unit_speech", CFG_DEF_SND_MENU_MUSIC);
  config.file->ReadBool(&config.snd_menu_music, "snd_menu_music", CFG_DEF_SND_GAME_MUSIC);
  config.file->ReadBool(&config.snd_game_music, "snd_game_music", CFG_DEF_SND_UNIT_SPEECH);

  // customize
  config.file->ReadStr(config.player_name, "player_name", CFG_DEF_PLAYER_NAME, true);
  config.file->ReadStr(config.address, "address", CFG_DEF_ADDRESS, true);
  config.file->ReadByteRange(&config.sensitivity, "sensitivity", 0, 100, CFG_DEF_SENSITIVITY);
